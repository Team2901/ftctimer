...
static const char *TAG = "wifi-captive-portal-httpd";

esp_event_loop_handle_t wifi_captive_portal_httpd_event_loop_handle;

ESP_EVENT_DEFINE_BASE(WIFI_CAPTIVE_PORTAL_HTTPD_EVENT);
...
static esp_err_t rest_common_get_handler(httpd_req_t *req)
{
    ...
    if (res != ESP_OK)
    {
        ESP_LOGE(TAG, "failed getting HOST header value: %d", res);

        switch (res)
        {
        case ESP_ERR_NOT_FOUND:
            ESP_LOGE(TAG, "failed getting HOST header value: ESP_ERR_NOT_FOUND: Key not found: %d", res);
            break;

        case ESP_ERR_INVALID_ARG:
            ESP_LOGE(TAG, "failed getting HOST header value: ESP_ERR_INVALID_ARG: Null arguments: %d", res);
            break;

        case ESP_ERR_HTTPD_INVALID_REQ:
            ESP_LOGE(TAG, "failed getting HOST header value: ESP_ERR_HTTPD_INVALID_REQ: Invalid HTTP request pointer: %d", res);
            break;

        case ESP_ERR_HTTPD_RESULT_TRUNC:
            ESP_LOGE(TAG, "failed getting HOST header value: ESP_ERR_HTTPD_RESULT_TRUNC: Value string truncated: %d", res);
            break;
    ...
    ESP_LOGI(TAG, "Got HOST header value: %s", req_hdr_host_val);

    const char redir_trigger_host[] = "connectivitycheck.gstatic.com";
    // TODO: Put code in here to handle the Redirect for IOS
    // see lots of good hints in:
    ...
    if (strncmp(req_hdr_host_val, redir_trigger_host, strlen(redir_trigger_host)) == 0)
    {
        ...
        ESP_LOGI(TAG, "Detected redirect trigger HOST: %s", redir_trigger_host);
    ...
    else if (req->user_ctx != NULL)
    {
        esp_err_t (*handler)(httpd_req_t *req) = req->user_ctx;
        esp_err_t res;
        res = handler(req);
        ESP_LOGI(TAG, "Called Registered Handler result=%03x", res);
        return res;
    }
    else
    {
        // This code really isn't needed or used...
        ESP_LOGI(TAG, "No redirect needed for HOST: %s", req_hdr_host_val);
...
static void start_httpd(void *pvParameter)
{
    ...
    httpd_uri_t common_get_uri = {
        ...
        .user_ctx = pvParameter};

    httpd_register_uri_handler(server, &common_get_uri);

    common_get_uri.method = HTTP_POST;
    httpd_register_uri_handler(server, &common_get_uri);
...
void wifi_captive_portal_httpd_task(void *pvParameter)
{
    while (1)
    {
        start_httpd(pvParameter);
